...
describe('miniatura adjunta del chat', () => {
    it('chip de cierre pequeño con área táctil invisible de 44 px', () => {
        ...
        expect(block).toMatch(/width:\s*24px;[\s\S]*height:\s*24px;/);
        expect(block).toContain('background: rgba(15, 23, 42, 0.78);');
    ...
    it('la miniatura lleva marco, sombra y el icono X pequeño', () => {
        ...
        expect(block).toContain('box-shadow: 0 2px 8px rgba(0, 0, 0, 0.28);');
        expect(block).toMatch(/flex:\s*0 0 128px;/);
        expect(SRC).toContain('.attachment-preview { flex-basis: 104px; width: 104px; height: 104px;');
