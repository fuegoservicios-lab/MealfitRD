...
describe('P1-TODAY-REMAINING — "Tu Menú" atenúa lo ya comido hoy (derivado, nunca persistido)', () => {
    ...
    it('an eaten slot genuinely disables Cambiar Plato and Me gusta — no click, no keyboard activation reaches their handlers', async () => {
        ...
        expect(swapBtn).toHaveAttribute('title', expect.stringContaining('Tus macros y micros de hoy'));
        expect(likeBtn).toHaveAttribute('title', expect.stringContaining('Tus macros y micros de hoy'));
        expect(swapBtn.getAttribute('aria-label') || swapBtn.getAttribute('title')).toMatch(/Tus macros y micros de hoy/);
        expect(likeBtn.getAttribute('aria-label') || likeBtn.getAttribute('title')).toMatch(/Tus macros y micros de hoy/);
