...
//
// [P1-PLAN-LOTE-223 · 2026-09-24] La casilla de cada componente pasa a decir «lo comí» (mueve las macros) y la
...
describe('P1-PHOTO-DEDUCTS — el escaner descuenta lo que el usuario confirma', () => {
    ...
    it('muestra los componentes detectados para confirmar, no los descuenta solos', async () => {
        ...
        // [P1-PLAN-LOTE-223] la Nevera es un interruptor propio, encendido como siempre
    ...
    it('[lote 223] con «Descontar de mi Nevera» apagado, los ingredientes se GUARDAN pero no se descuentan', async () => {
        await _scan();
    ...
    describe('con la Nevera apagada', () => {
        ...
        it('si el perfil la apaga con el escáner montado, «Descontar de mi Nevera» se va; y vuelve', async () => {
            ...
            expect(screen.getByText('Ingredientes que detectamos')).toBeInTheDocument();
            expect(screen.queryByText(/Descontar de mi Nevera/i)).not.toBeInTheDocument();
            // [P1-PLAN-LOTE-223] la casilla de cada componente es «lo comí»: sigue ahí con la Nevera apagada
            expect(screen.getByLabelText('Incluir huevo')).toBeInTheDocument();
