...
const FormattedRecipeStep = ({ step, index }) => {
  ...
  const getSectionInfo = (text) => {
    ...
    if (lowerText.startsWith("mise en place:"))
      return { title: "Mise en place", color: "#00B4D8", icon: <ChefHat /> };
    if (
      lowerText.startsWith("el toque de fuego:") ||
      lowerText.startsWith("toque de fuego:")
    )
      return { title: "El Toque de Fuego", color: "#F97316", icon: <Flame /> };
    if (lowerText.startsWith("montaje:"))
      return { title: "Montaje", color: "#8B5CF6", icon: <Utensils /> };
    return null;
  };
  ...
  if (sectionTitle) {
    ...
    const prefixRegex =
      sectionTitle.toLowerCase() === "toque de fuego" ||
      sectionTitle.toLowerCase() === "el toque de fuego"
...
const FormattedLargeStep = ({ text, currentStep, isLastStep, isMobile }) => {
  const getSectionInfo = (t) => {
    ...
    if (lowerT.startsWith("mise en place:"))
      return {
        title: "Mise en place",
        color: "#00B4D8",
    ...
    if (
      lowerT.startsWith("el toque de fuego:") ||
      lowerT.startsWith("toque de fuego:")
    )
      return {
        title: "El Toque de Fuego",
        color: "#F97316",
        icon: <Flame size={32} />,
      };
    if (lowerT.startsWith("montaje:"))
      return {
        title: "Montaje",
        color: "#8B5CF6",
      ...
      >
        ...
        {isLastStep ? (
          ...
          >
            ...
            {isSubmitting ? "Cargando..." : "Terminar"}
          </button>
        ) : (
          <button
            onClick={handleNext}
...
const Recipes = () => {
  ...
  const handleCookClick = async (meal) => {
    ...
    const loadingToast = toast.loading(
      `El Chef AI está detallando los pasos para ${meal.name}...`,
    );
      ...
      if (response.ok && data.success && data.expanded_recipe) {
        ...
        if (planData) {
          try {
            ...
            console.error("Error setting plan to LS/DB:", e);
          }
        }
        ...
        toast.error(
          data.detail || "No se pudo expandir la receta. Abriendo original.",
          { id: loadingToast },
        );
        setCookingRecipe(meal);
      }
  ...
  const handleLogConsumption = async (recipe) => {
    ...
    try {
      ...
      if (token) {
        ...
        jwt =
          parsed?.currentSession?.access_token || parsed?.access_token || token;
      }
      ...
      toast.success(`¡"${recipe.name}" registrada exitosamente!`, {
        id: toastId,
      });
    } catch (error) {
      console.error(error);
      toast.error("No se pudo registrar la comida. Intenta de nuevo.", {
        id: toastId,
      });
    } finally {
      setCookingRecipe(null);
    ...
    >
      ...
      <div
        style={{
          ...
          height: "40vw",
          background:
            "radial-gradient(circle at center, var(--secondary) 0%, transparent 60%)",
          filter: "blur(100px)",
          transform: "translateZ(0)",
          borderRadius: "50%",
          opacity: 0.1,
      ...
      <AnimatePresence>
        {cookingRecipe && (
          <CookingModeOverlay
            ...
            onComplete={handleLogConsumption}
          />
        )}
      </AnimatePresence>
      <div
          ...
          >
              ...
              >
                {planData.days.map((dayObj, idx) => (
                  <button
                    ...
                    style={{
                      flex: 1,
                      padding: isMobile ? "0.6rem 0.15rem" : "0.85rem 1rem",
            ...
            {(() => {
              const planDays = planData.days || [
                { day: 1, meals: planData.meals || planData.perfectDay || [] },
              ];
              ...
              const validMeals =
                dayObj.meals?.filter((meal) => {
                  if (formData?.skipLunch) {
                    const isLunch =
                      meal.meal.toLowerCase().includes("almuerzo") ||
              ...
              const currentMealIndex = Math.min(
                activeMealIndex,
                validMeals.length - 1,
              );
              ...
              return (
                ...
                >
                  ...
                  <div
                    style={{ padding: "0", minWidth: 0, overflow: "hidden" }}
                  >
                    <AnimatePresence mode="wait">
                      <motion.div
                      ...
                      >
                            ...
                            activeMeal.recipe.length > 0 && (
                              <button
                                ...
                                style={{
                                  ...
                                  transition: "all 0.2s",
                                  boxShadow:
                                    "0 8px 16px -4px rgba(15, 23, 42, 0.4)",
                                  opacity: isExpanding ? 0.7 : 1,
                                }}
                              >
                                <Play size={18} fill="white" />{" "}
                                {isExpanding ? "Generando..." : "Cocinar"}
                              </button>
                            )}
                          <button
                            data-html2canvas-ignore="true"
                        ...
                        >
                          ...
                          >
                            {/* Sleek Macros Design */}
                            {activeMeal.protein !== undefined &&
                              activeMeal.protein > 0 && (
                                <div
                                  style={{
                                    background: "var(--bg-page)",
                          ...
                          <div style={{ flex: 1, width: "100%", minWidth: 0 }}>
                            ...
                            activeMeal.recipe.length > 0 ? (
                              ...
                              >
                                ...
                                >
                                  No hay pasos detallados. Guíate de la
                                  descripción general.
                                </p>
                              </div>
                            )}
