...
// [P1-PLAN-LOTE-225 · 2026-09-24] La palabra española es lo que `handleChipAdd` resuelve contra el catálogo; lo que se
...
const PantryPage = () => {
    ...
    const handleChipAdd = (word) => {
        ...
        if (!hit) {
            ...
            // seleccionado sigue siendo `m.name`. [P1-PLAN-LOTE-225] Y los otros tres idiomas: `formasDeBuscar`.
    ...
    const suggestedMasterItems = useMemo(() => {
        ...
        // [P2-I18N-BUSCADOR-CATALOGO-PUENTE-EN-1-DE-4] ver arriba. [P1-PLAN-LOTE-225] los cinco idiomas, sin acentos.
    ...
    const renderRow = (item) => {
        ...
        // [P1-PLAN-LOTE-225] El alimento en el idioma del usuario, una vez por fila (el dato sigue en español).
    ...
    const renderMobileCard = (item) => {
        ...
        const catInk = zoneInk(getZoneForCategory(item.master_ingredients?.category));
        const atFloor = item.quantity <= 1;
        // [P1-PLAN-LOTE-225] El alimento en el idioma del usuario, una vez por tarjeta (el dato sigue en español).
        const nombreVisible = nombreDelAlimento(item.ingredient_name);
        const badge = getShelfLifeBadge(item, t, tn);
        const badgeStyle = badge ? getShelfLifeBadgeStyle(badge.severity) : null;
        // [P1-PANTRY-LOW-IS-A-LIE · 2026-08-09] El estado de atención de la fila
