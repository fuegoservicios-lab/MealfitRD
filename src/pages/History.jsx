...
const History = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const validPlans = (data || []).filter(
        (plan) => plan.name && plan.calories > 0,
      );
      setPlans(validPlans);
    } catch (error) {
  ...
  const handleEditSave = async (e, plan) => {
    ...
    try {
      ...
      setPlans(
        plans.map((p) =>
          p.id === plan.id ? { ...p, name: tempName.trim() } : p,
        ),
      );
  ...
  const getSmartTags = (plan) => {
    ...
    const allergies =
      data.allergies || assessment.allergies || assessment.intolerances || [];
    if (Array.isArray(allergies)) {
      if (allergies.includes("lactose") || allergies.includes("dairy"))
        tags.push("Sin Lácteos");
    ...
    <>
      <div className={styles.container}>
        <div className={styles.headerRow}>
          <div>
            <h1 className={styles.title}>Librería de Planes</h1>
          ...
          {!loading && plans.length > 0 && (
            <span className={styles.planCount}>
              {plans.length}{" "}
              {plans.length === 1 ? "plan guardado" : "planes guardados"}
            </span>
          )}
        </div>
      </div>
                  ...
                  <div className={styles.planInfo}>
                    {isEditing === plan.id ? (
                      <div className={styles.planNameRow}>
                        ...
                        />
                        <button
                          onClick={(e) => handleEditSave(e, plan)}
            ...
            >
              ...
              <div className={styles.modalHeader}>
                <div>
                  <h2 className={styles.modalTitle}>
                    {selectedPlan.name || "Detalles del Plan"}
                  </h2>
              ...
              <div className={styles.modalBody}>
                ...
                <div className={styles.macrosGrid}>
                  <div
                    className={`${styles.macroCard} ${styles.macroCardOrange}`}
                  >
                    <Flame
                      size={18}
                      color="#EA580C"
                    ...
                    />
                    <div className={styles.macroValueOrange}>
                      {selectedPlan.calories}
                    </div>
                    <div className={styles.macroLabelOrange}>kcal</div>
                  </div>
                  <div
                    className={`${styles.macroCard} ${styles.macroCardBlue}`}
                  >
                    <Dumbbell
                      size={18}
                      color="#2563EB"
                      style={{ marginBottom: "0.4rem" }}
                    />
                    <div className={styles.macroValueBlue}>
                      {selectedPlan.macros?.protein || "—"}
                    </div>
                    <div className={styles.macroLabelBlue}>Proteína</div>
                  </div>
                  <div
                    className={`${styles.macroCard} ${styles.macroCardGreen}`}
                  >
                    <Wheat
                      size={18}
                      color="#059669"
                      style={{ marginBottom: "0.4rem" }}
                    />
                    <div className={styles.macroValueGreen}>
                      {selectedPlan.macros?.carbs || "—"}
                    </div>
                    <div className={styles.macroLabelGreen}>Carbos</div>
                  </div>
                  <div
                    className={`${styles.macroCard} ${styles.macroCardPink}`}
                  >
                    <Droplet
                      size={18}
                      color="#EC4899"
                      style={{ marginBottom: "0.4rem" }}
                    />
                    <div className={styles.macroValuePink}>
                      {selectedPlan.macros?.fats || "—"}
                    </div>
                ...
                <div className={styles.menuList}>
                  {(
                    selectedPlan.plan_data?.days?.[selectedDay]?.meals ||
          ...
          >
            ...
            >
              ...
              <p className={styles.confirmText}>
                Tu plan actual será reemplazado por{" "}
                <strong>{confirmRestore.name || "este plan"}</strong>. Esta
        ...
        {confirmDelete && (
          ...
          >
            ...
            >
              ...
              <p className={styles.confirmText}>
                El plan <strong>{confirmDelete.name || "Seleccionado"}</strong>{" "}
                será borrado permanentemente de tu historial. Esta acción no se
                puede deshacer.
              </p>
              <div className={styles.confirmActions}>
                <button
