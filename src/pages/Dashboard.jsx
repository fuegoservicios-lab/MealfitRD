...
// [P1-PLAN-LOTE-224] Las líneas «2 unidad de Huevo» que devuelve el servidor, en el idioma del usuario.
...
// [P1-PLAN-LOTE-223 · 2026-09-24] El escáner de comida, perezoso y montado al pedirlo (ya se montaba así): el mismo
...
// [P1-PLAN-LOTE-224 · 2026-09-24] Las frases fijas con que `/api/plans/restock` explica un fallo (routers/plans.py
...
const DashboardInner = () => {
    ...
    const {
        ...
        hydrateLatestPlan,
        // [P1-PLAN-LOTE-224 · 2026-09-24] Tras «Arreglar este día»: volver a por la traducción del plato nuevo.
    ...
    const handleFixSodiumDay = async (allowNewIngredients = null) => {
        ...
        try {
            const resp = await fetchWithAuth(`${API_BASE}/api/plans/${planData.id}/fix-sodium-day`, {
                ...
                body: JSON.stringify({
                    ...(Array.isArray(allowNewIngredients) && allowNewIngredients.length > 0
                        ...
                        : {}),
                    // [P1-PLAN-LOTE-224 · 2026-09-24] Para `new_meal_display`: el plato nuevo en el idioma del usuario.
                    locale: _dashLocale,
                }),
            });
            let result = null;
            ...
            if (result?.fixed === true) {
                // [P1-PLAN-LOTE-224 · 2026-09-24] Los dos platos del aviso en el idioma del usuario: el viejo con su
                ...
                try { esperarTraduccionDelDia?.(Number(result.day)); } catch (_) { /* no-op */ }  // [P1-PLAN-LOTE-224]
    ...
    // [P1-PLAN-LOTE-224 · 2026-09-24] Cómo se LEE el nombre de algo registrado en el diario (el plato del plan en el
    ...
    const handleRestock = async (opts = {}) => {
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                // [P1-PLAN-LOTE-224 · 2026-09-24] El motivo en el idioma del usuario (el servidor lo escribe en español).
    ...
    // [P1-PLAN-LOTE-224 · 2026-09-24] Los suplementos del día los escribe el modelo en español y no viven en `_display`:
            ...
            <div className="main-grid">
                ...
                <div className={`meals-container${dayHasMealCards ? '' : ' meals-container--sin-filas'}`} style={{ flex: 2, alignSelf: 'start' }}>
                    ...
                    {(() => {
                        ...
                        // pausado.
                        const _reasonCopy = {
                            // [P1-CTA-RUTAS-EXISTEN · 2026-08-21] Los `url` de estos
                            // banners los consume `navigate(_copy.url)`, asi que TIENEN
                            // que ser rutas declaradas en App.jsx. Cinco apuntaban a
                            // `/inventory` y uno a `/diary`, que no existen: caian en el
                            // catch-all `path="*"` -> NotFound. Y son justo los CTA que
                            // piden una accion urgente. La Nevera real es
                            // `/dashboard/pantry`; el diario vive DENTRO de `/dashboard`
                            // (TrackingProgress), no en una ruta propia.
                            empty_pantry: { title: t('Tu próximo bloque está pausado'), body: t('Tu nevera está vacía. Añade ingredientes para que generemos los próximos días.'), cta: t('Actualizar nevera'), url: '/dashboard/pantry' },
                            empty_pantry_proactive: { title: t('Tu próximo bloque está pausado'), body: t('Tu nevera está vacía. Añade ingredientes para que generemos los próximos días.'), cta: t('Actualizar nevera'), url: '/dashboard/pantry' },
                            ...
                            persistent_drift: { title: t('Validando tu inventario'), body: t('Detectamos diferencias persistentes con tu inventario. Refrescando…'), cta: t('Abrir nevera'), url: '/dashboard/pantry' },
                        };
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        {(() => {
                                                            ...
                                                            // [P1-PLAN-LOTE-224 · 2026-09-24] El backend
