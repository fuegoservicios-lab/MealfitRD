...
const DASHBOARD_STOP_WORDS = ['picada', 'picado', 'en tiras', 'en cubos', 'rallado', 'rallada',
...
const DASHBOARD_STOP_WORDS_REGEX = new RegExp('\\b(' + DASHBOARD_STOP_WORDS.join('|') + ')\\b', 'gi');
const DASHBOARD_MEAT_PREFIX_REGEX = /^(pechuga|filete|muslo|trozo|chuleta|pieza|corte|ración|racion|porción|porcion|filetico|medallón|medallones|carne)s?\s+(de|del)\s+/i;
const DASHBOARD_INGREDIENT_IRREGULARS = {
    'nueces': 'nuez',
    'aves': 'ave',
    'maices': 'maiz',
    'arroces': 'arroz',
...
const Dashboard = () => {
    ...
    const buildDeltaShoppingList = useCallback((shoppingList, inventoryOverride = null) => {
        ...
        const normalizeNameAlt = (name) => {
            ...
            n = n.replace(DASHBOARD_MEAT_PREFIX_REGEX, '').trim();
            ...
            n = n.replace(DASHBOARD_STOP_WORDS_REGEX, '');
            n = n.replace(/,/g, '').replace(/\s+/g, ' ').trim();

            return n.split(/\s+/).map(w => {
                 if (DASHBOARD_INGREDIENT_IRREGULARS[w]) return DASHBOARD_INGREDIENT_IRREGULARS[w];
